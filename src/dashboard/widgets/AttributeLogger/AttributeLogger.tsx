...
class AttributeLogger extends Component<Props> {
  ...
  reloadLog() {
    ...
    //in this value
    if ((this.props.inputs.logIfChanged === false) || this.isDifferentToLastMessage(currMessage, valueLog)) {
      ...
      if (currMessage !== "") {
        ...
        if (this.props.inputs.linesDisplayed - valueLog.length > 0) {
          newLog = [...valueLog, { "timestamp": attribute.timestamp, "value": currMessage }]
        } else {
           newLog = [...valueLog.slice(1), { "timestamp": attribute.timestamp, "value": currMessage }]
        }
